...
def read_candlist(candlist_file):
    with open(candlist_file, 'r', encoding='utf-8') as f:
        candlist = []
        for line in f:
...
def read_corids(cor_file):

    with open(cor_file, 'r', encoding='utf-8') as f:
        cor_ids = []
        for line in f:
            cor_ids.append(int(line.strip()))
...
def gen_randsamp(origsample, candlist, numsamps):
    randsamps = []
    numcands = len(candlist)
    candss = ["","","",""]
    candids = [0,1,2,3]
    candss[origsample.label] = origsample.endings[origsample.label]
    candids.remove(origsample.label)
    for i in range(numsamps):
        candss[candids[0]] = candlist[random.randint(0,numcands-1)]
        candss[candids[1]] = candlist[random.randint(0, numcands-1)]
        candss[candids[2]] = candlist[random.randint(0, numcands-1)]
        randsamps.append(SwagExample(
            swag_id = origsample.swag_id,
            context_sentence = origsample.context_sentence,
            start_ending = origsample.start_ending, # in the swag dataset, the
                                         # common beginning of each
                                         # choice is stored in "sent2".
            ending_0 = candss[0],
            ending_1 = candss[1],
...
def main():
    ...
    parser.add_argument("--num_cands",
                        default=50,
                        type=int,
                        help="Number of generated random samples per example.")
    ...
    candlist = read_candlist(candspath)
    cor_ids = read_corids(corpath)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for corid in tqdm(cor_ids, desc="Originals:"):
            origexample = eval_examples[corid]
            filter_examples = gen_randsamp(origexample, candlist, args.num_cands)
            eval_features = convert_examples_to_features(
                filter_examples, tokenizer, args.max_seq_length, True)
            ...
            for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
                ...
                if tmp_eval_accuracy ==0:
                    ...
                    tempexample = filter_examples[evalcount]
                    print(corid,",",origexample.swag_id,","+tempexample.context_sentence+","+tempexample.start_ending
                          +","+tempexample.endings[origexample.label]+","+tempexample.endings[pred[0]])
                    """
